...
class COCODemo(object):
    ...
    def create_mask_map(self, image, predictions):
        ...
        labels = predictions.get_field("labels") # Shape: [num_instances]. Values in {1, 2}
        # Note: masks/labels are in descending score order

        obj_id = 2 # start at 2
        result = np.zeros((H,W))
        for idx in range(len(labels))[::-1]: # from lowest score to highest
            label = self.CATEGORIES[labels[idx]]
            if label == 'dining table': # leave out table for now
                continue
            mask = masks[idx, 0] # Shape: [H x W]
            result[mask == 1] = obj_id
            obj_id += 1
